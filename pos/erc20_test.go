...
func TestERC20_Approve(t *testing.T) {
	...
	hash, err := erc20.Approve(context.Background(), big.NewInt(10000), privateKey)
	assert.NoError(t, err)
...
func TestERC20_DepositFor(t *testing.T) {
	...
	erc20 := client.ERC20(RootDummyERC20, types.Root)
	hash, err := erc20.DepositFor(context.Background(), big.NewInt(10000), privateKey)
	assert.NoError(t, err)
	t.Log("txHash", hash.String())
}
...
func TestERC20_Withdraw(t *testing.T) {
	...
	hash, err := erc20.Withdraw(context.Background(), big.NewInt(10000), privateKey)
	assert.NoError(t, err)
	t.Log("txHash", hash.String())
}
...
func TestERC20_Balance(t *testing.T) {
	...
	assert.NoError(t, err)
	childBalance, err := client.ERC20(ChildDummyERC20, types.Child).BalanceOf(context.Background(), address)
	assert.NoError(t, err)
